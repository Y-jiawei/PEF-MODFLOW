% This function is called when the optimum number of horizons is 8
 function  H8(image1,image2,soil_depth,N,sumo,output,idx,S)
I=imread('image1.jpg'); 
f=imread('image2.jpg');
f=rgb2lab(f);
fff=f(:,:,1);
...
imgx=img1+img2+img3+img4+img5+img6+img7+img8;
for  i=1:m
    for j=1:n      
if imgx(i,j)==0
    imgx(i,j)=[NaN];
...
YY7=(ones(1,n)*MM7)';YY8=(ones(1,n)*MM8)';
...
HH=[MM1,MM2,MM3,MM4,MM5,MM6,MM7,MM8];
HH=sort(HH);
