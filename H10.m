% This function is called when the optimum number of horizons is 10
 function  H10(image1,image2,soil_depth,N,sumo,output,idx,S)
I=imread('image1.jpg'); 
f=imread('image2.jpg');
f=rgb2lab(f);
fff=f(:,:,1);
...
if img1(i,j)==1
    ...
    if img10(i,j)==1
    img10(i,j)=img10(i,j)+9;
    end      
    
    end
end
imgx=img1+img2+img3+img4+img5+img6+img7+img8+img9+img10;
for  i=1:m
    for j=1:n      
if imgx(i,j)==0
    imgx(i,j)=[NaN];
...
if M9>1;
 ...
 % Perfecting the boundaries
% Boundary 10
N10=max(x10(:,1)); % Calculate the maximum number of rows
M10=min(x10(:,1)); % Calculate the minimum number of rows
[i10,j10]=find(x10==N10);% Locking the upper boundary
ii10=i10(1,1);
xx10=x10(1:1:ii10);  % Dividing upper boundary row values
yy10=y10(1:1:ii10);  % Divided upper boundary column values
mm10=mean(yy10); % Calculate the mean of existing column values 
if M10>1;
    A10=M10-1;
    B10=[1:1:A10]';% Generate the missing prefix for x
    b10=ones(1,A10)*mm10; 
    b10=b10';        % Generate the missing prefix for y
else
    ...
    if N10<n;
       C10=N10+1 ;
        D10=[C10:1:n]';% Generate the missing suffix for x
        p10=n-N10;
        d10=ones(1,p10)*mm10;
        d10=d10';      %  Generate the missing suffix for y
    else
            d10=[]; D10=[];
            
    end

    xx10=[B10;xx10;D10];% Synthetic x
    yy10=[b10;yy10;d10];% Synthetic y 
    
% Locking the uppermost boundary
Y1=min(yy1); Y2=min(yy2); Y3=min(yy3);Y4=min(yy4); 
Y5=min(yy5); Y6=min(yy6);Y7=min(yy7);Y8=min(yy8);
Y9=min(yy9);Y10=min(yy10);
...
if q==10
    xx10=[1:1:n];% Limit the uppermost boundary row
    yy10=ones(1,n);% Limit the uppermost boundary column
end
...
arr10=vpa(HHH10*soil_depth/m,2);
