% This function is called when the optimum number of horizons is 7
 function  H7(image1,image2,soil_depth,N,sumo,output,idx,S)
I=imread('image1.jpg'); 
f=imread('image2.jpg');
f=rgb2lab(f);
fff=f(:,:,1);
...
imgx=img1+img2+img3+img4+img5+img6+img7;
for  i=1:m
    for j=1:n      
if imgx(i,j)==0
    imgx(i,j)=[NaN];
...
Y5=min(yy5); Y6=min(yy6);Y7=min(yy7);
...
MM5=mean(yy5); MM6=mean(yy6); MM7=mean(yy7);
...
HH=[MM1,MM2,MM3,MM4,MM5,MM6,MM7];
HH=sort(HH);
HHH1=HH(:,[1]); HHH2=HH(:,[2]); HHH3=HH(:,[3]); HHH4=HH(:,[4]);
HHH5=HH(:,[5]); HHH6=HH(:,[6]); HHH7=HH(:,[7]);
